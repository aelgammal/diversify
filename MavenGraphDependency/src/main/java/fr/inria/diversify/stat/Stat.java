...
public class Stat {
    ...
    public void writeGeneralStat(String fileName) throws IOException {
        Map<String, Integer> dd = g.artifactDependencyDistribution(false);
        Map<String, Integer> alldd = g.artifactDependencyDistribution(true);
        Map<String, Integer> du = g.artifactUsedDistribution(false);
        Map<String, Integer> alldu = g.artifactUsedDistribution(true);
        Map<String, Integer> ad = g.artifactDistribution();

        FileWriter fw = new FileWriter(fileName+"_artifactDistribution.csv");
        BufferedWriter bw = new BufferedWriter(fw);
        bw.write("Artifact;number;directDependency;allDependency;directUsed;allUsed\n");
        for (String s : dd.keySet()) {

            bw.write(s+";"+ad.get(s)+";"+((double)dd.get(s)/(double)ad.get(s))+";"+((double)alldd.get(s)/(double)ad.get(s)));
            if(du.containsKey(s))
                bw.write(";"+((double)du.get(s)/(double)ad.get(s))+";"+((double)alldu.get(s)/(double)ad.get(s))+"\n");
            else
                bw.write(";0;0\n");
        }
